...
def data_splitter(center_list, dists_min, signal, dists_max, prints=True):
    ...
    if((len(dists_max) == 0) or (len(dists_min) == 0)):
        ...
        # print('max_distLL:' , max_dist)
        # print(max_dist)

    half_length = 50 # min(150, max_dist) # 90
...
def helper_func_v2(thermal, t_arr, gt_breathing, th=0.4, dmin=25, dmax=25, dmax_func=None, dmin_func=None,  mode='mean', plot=False, prints=False, include_edges=True, motion_th=4.5 , is_radar=False):
        ...
        for p, sig in enumerate(signal_comps):
            if(len(sig[0]) != 0):
                ...
                if(std != 0):
                    ...
                    if(gt_signal is not None):
                        pred_chunk = helper_func_v2(new_signal, time_arr[sig[1][0]:sig[1][1]], gt_signal[sig[1][0]:sig[1][1]], th=th, dmin=dmin, dmax=dmax, dmax_func=dmax_func, dmin_func=dmin_func, mode=mode, plot=plot, prints=prints, motion_th=motion_th, is_radar=is_radar)
                    else:
                        pred_chunk = helper_func_v2(new_signal, time_arr[sig[1][0]:sig[1][1]], None, th=th, dmin=dmin, dmax=dmax, dmax_func=dmax_func, dmin_func=dmin_func, mode=mode, plot=plot, prints=prints, motion_th=motion_th, is_radar=is_radar)
